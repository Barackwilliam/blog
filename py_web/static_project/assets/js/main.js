...
$(document).ready(function(){
    ...
    $(class_name).click(function(e){
        ...
        if (atr.includes('false')){
            class_name.children().children('.icon-badge-icon').attr('style','color:white;');
        }
        else{
            class_name.children().children('.icon-badge-icon').attr('style','color:dimgray;');
        }
    })
},3000);
})
...
function edit_username(val, usr_id, extra){
    var this_ = $(val + usr_id);
    var z = this_.attr('data-value');
    var edt_ = $(val + usr_id + 'Edit');
    if (extra == ''){
        this_.remove();
        edt_.html('');
        $('#commbox' + usr_id).append("<textarea class='form-control' id='usertext" + usr_id + "' name='message' rows='1' cols='15' data-value='" + z + "' required>" + z + "</textarea>");
        $('#editabletext' + usr_id + 'button').attr('class','btn btn-sm btn-success mt-0').html("<i class='fa fa-check'></i>");
        $('#canceltext' + usr_id + 'button').attr('class','btn btn-sm btn-danger mt-0').html("<i class='fa fa-trash'></i>");
    }
    else if (extra == 'cancel'){
        this_.remove();
        edt_.html('edit user-name');
        $('#commbox' + usr_id).append("<small class='p-0 m-0' id='usertext" + usr_id + "' data-value='" + z + "'></small>");
        $('#editabletext' + usr_id + 'button').removeAttr('class').html('');
        $('#canceltext' + usr_id + 'button').removeAttr('class').html('');
    }
    else if (extra == 'sbmit'){
            $('#editabletext' + usr_id + 'button').attr('class','btn btn-sm btn-light disabled').html("save <span class='spinner-border spinner-border-sm' role='status' aria-hidden='true'></span>");
            event.preventDefault();
            var this_ = $('#commbox' + usr_id);
            ...
            $.ajax({
                ...
                data : serializedData,
                success : function(data){
                    var this_ = $('#usertext' + usr_id);
                    this_.remove();
                    alert('You changed username. Refresh this page.')
                    window.location.href = '/account/' + data.name + '/dashboard/';

                },
                error : function(data){
                    var msz = data.responseJSON['msz'];
                    if (msz != undefined && data.responseJSON['msz'] == msz){
                        alert(data.responseJSON['msz']);
                    }
                    location.reload();
...
function editable_comment(val, comm_id, extra){
    ...
    if (extra == ''){
        ...
        $('#commbox' + comm_id).append("<textarea class='form-control' id='commtext" + comm_id + "' name='message' rows='5' cols='50' required>" + z + "</textarea>");
...
function go_to_follow(name){
    window.location.href = '/' + name + '/follow_to/';
}
    ...
    $('.show-ntf').click(function(e){
        ...
        $.ajax({
            ...
            success:function(context){
                var ntf = "";
                    ntf += "<div class='icon-badge-container icon-badge-click mt-2' onclick=notification_engine(event,'icon-badge-click');return  data-href='/" + context.nav_username + "/notifications/' data-toggle='dropdown' aria-haspopup='true' aria-expanded='false'><a href='#'><i class='far fa-bell icon-badge-icon' style='color:dimgray;'></i></a>";
                    ...
                    ntf += "</ul></div><p class='list-group-item text-center list-group-item-dark mb-0'><a href='/notification/' class='d-inline-block'> See all </a></p></div>";
...
$(document).ready(function(){
    ...
    if (url_link){
        ...
        if (select_course){
            var folo = document.getElementById(url_link + '-tab');
            folo.click();
        }
...
function reply_Public_comment(event,val,extra,last){
        event.preventDefault();
        ...
        $.ajax({
            ...
            success: function(data){
                    ...
                    if (val.includes('qes')){
                        ...
                        $('.get_discuss_ans' + last).click();
                    }
                    else{
                        $('.get_discuss_' + val.toString()).click();
                    }
                    $('.get_comment').click();
            },
            error : function(data){
...
function refresh(data){
    ...
    for (people=0;people<data.count;people++){
        prevstate = "";
        var uname=data['useracc'][people]['username__username'];
        ...
        var cnt = data['follower_count_' + uid.toString()];
        prevstate = prevstate + "<div class='card mb-0 mt-0 pt-0 pb-0 border-0' style='max-width: 500px;'><div class='row no-gutters mb-0 mt-0 pt-0 pb-0'><div class='col-sm-1 py-3'>"
         var pto = 'follower_profile_' + uid.toString();
         ...
         prevstate = prevstate + "<div class='col-sm-8'><div class='card-body'><small><a class='card-title' href='/account/" + uname + "/dashboard/'" + "><strong>"; 
         if(first_name){
         ...
         prevstate = prevstate + "</strong></a></small><p class='card-text m-0 p-0'>";
             if (data['useracc'][people]['interests']){
                 prevstate = prevstate + data['useracc'][people]['interests']
             }
         prevstate = prevstate + "</p><small class='text-muted'>followed by </small><small data-value='pop-followers-number" + data['useracc'][people]['id'] + "' class='text-muted pop-followers-number" + uid + "'>"
         ...
         prevstate = prevstate + "</small><small class='text-muted'> people</small></div></div><div class='col-sm-2'><div class='card-body'>";
             if (data['username'] != uname){
                 ...
                 prevstate = prevstate + "<button onclick='follow_and_comment_like(event," + uid + ");return' data-href='/account/" + uname + "/follow/' class='btn btn-sm btn-outline-dark followbtn2" + uid + "' id='followbtn2" + uid + "'>";
                     
                     if (data['follower_inside_' + uid.toString()]=="unfollow"){
...
function follow_and_comment_like(event, val){
    var this_ = $('.followbtn2' + val.toString());
    if (this_.length == 0) {
        this_ = $('#comment_likes_' + val);
    }
    var token = $('.followers-numberid').attr('token');
    event.preventDefault();
    var this_ = $(this_);
    this_.html("<span class='spinner-border spinner-border-sm disabled' role='status' aria-hidden='true'></span>");
    ...
    $.ajax({
        ...
        success: function(data){
            if (this_.hasClass('comment_like')){
                ...
                this_count = $('.comment_like_count_' + val.toString());
                if (data.count > 0) {
                ...
                this_.text(data.condition);
                this_part_pop = $('.pop-followers-number' + val.toString());
...
function get_comment_func(event){
   $(document).ready(function(){
    ...
    comm_state += "(" + c_date_created + ")"; 
                    if (req_username == post_author_username || req_username == c_username){
                        comm_state += "<strong class='dropdown'><strong class='text-black dropbutton" + c_id + "main' id='dropdownMenuButton' data-toggle='dropdown' aria-haspopup='true' aria-expanded='false'> <a class='text-decoration-none' href='#'>. . .</a></strong><small class='dropdown-menu' aria-labelledby='dropdownMenuButton'>";
            ...
            comm_state += "' alt='pt'><div class='media-body'><p class='p-0 m-0'><a class='c-hover comment-hover-func-r" + cr_id + "' href='mailto:" + cr_username + "' data-href='/account/" + cr_username + "/comment_hover_user/' onmouseover=hover_func(" + cr_id + ",'r') data-toggle='tooltip_r" + cr_id + "' data-html='true' title='" + cr_username + "'>";
                    ...
                    comm_state += "</a><small class='text-muted'>";
                        ...
                        if (req_username == post_author_username || req_username == cr_username){
                            comm_state += "<strong class='dropdown'><strong class='text-black dropbutton" + cr_id + "reply' id='dropdownMenuButton' data-toggle='dropdown' aria-haspopup='true' aria-expanded='false'> <a class='text-decoration-none' href='#'>. . .</a></strong><small class='dropdown-menu' aria-labelledby='dropdownMenuButton'>";
    ...
    $(document).ready(function(){
        ...
        function on_error(data){
           if (data["responseJSON"]["pop_login"] == true){
               ...
               else if(data['responseJSON']['false_message']['first_name']=='Invalid First name'){
                   alert(data["responseJSON"]["false_message"]['first_name']);
               }
               else if(data['responseJSON']['false_message']['last_name']=='Invalid Last name'){
                alert(data["responseJSON"]["false_message"]['last_name']);
                }
